...
//					mapStyleOptions.setStyleId("your id");
	...
	private void setMapCustomStyleFile(Context context) {
		...
		String styleName_extra ="style_extra.data";
		InputStream inputStream = null;
		InputStream inputStream_extra = null;
		try {
			inputStream = context.getAssets().open(styleName);
			byte[] b = new byte[inputStream.available()];
			inputStream.read(b);
			inputStream_extra = context.getAssets().open(styleName_extra);
			byte[] c = new byte[inputStream_extra.available()];
			inputStream_extra.read(c);

			if(mapStyleOptions != null) {
				// 设置自定义样式
				mapStyleOptions.setStyleData(b);
				mapStyleOptions.setStyleData(c);
			}
			...
			try {
				...
				if (inputStream_extra != null) {
					inputStream_extra.close();
				}
